...
// 计算线的位置
  const crossLineHeight = ctx.canvas.height * 4 / 8;  // 使用新的变量名
  
  const crossLineWidth2 = ctx.canvas.width * 1; // 这是一个示例值，您需要根据实际情况进行调整
  const crossLineWidth1 = ctx.canvas.width * 0.40; // 这是一个示例值，您需要根据实际情况进行调整
  ...
  for (let i = 0; i < scores_data.length; ++i) {
	...
	const m = ctx.canvas.height / (crossLineWidth2 - crossLineWidth1);
	const centerY = (y1 + y2) / 2;
	const centerX = (x1 + x2) / 2;

	const yLineAtCenterX = m * (centerX - crossLineWidth1);

	if(centerY >= crossLineHeight && centerY > yLineAtCenterX) {
...
// ... (原来的代码)
	// 绘制直线
 	 // 绘制直线
	ctx.strokeStyle = "rgba(255,255,1,1)"; // 设置为浅蓝色
	ctx.lineWidth = 1; 
	ctx.setLineDash([5, 5]);  
	ctx.beginPath();
	ctx.moveTo(0, crossLineHeight);  // 使用新的变量名
	ctx.lineTo(ctx.canvas.width, crossLineHeight);  // 使用新的变量名
	ctx.stroke();
	
	// 绘制垂直直线
	ctx.strokeStyle = "rgba(255,255,1,1)"; // 设置为浅蓝色
	ctx.lineWidth = 1; 
	ctx.setLineDash([5, 5]); // 设置线的宽度
	ctx.beginPath();
	ctx.moveTo(crossLineWidth1, 0); // 从画布的顶部开始
	ctx.lineTo(crossLineWidth2, ctx.canvas.height); // 画到画布的底部
	ctx.stroke(); // 完成绘制
  ...
  // 展示计数结果
	...
	ctx.fillText(`In  \u2193: ${lineCrossCounts.inCar}`, 10, crossLineHeight - offsetText);  // 使用新的变量名
	ctx.fillText(`Out \u2191: ${lineCrossCounts.outCar}`, ctx.canvas.width - 100, crossLineHeight - offsetText);  // 使用新的变量名
	...
	// 定义所有类别和对应的颜色
	const allClasses = [
	  { name: "bicycle", color: "#C71585" },
	  { name: "car", color: "#27408B" },
	  { name: "motorcycle", color: "#458B74" },
	  { name: "bus", color: "#9400D3" },
	  { name: "train", color: "#838B83" },
	  { name: "truck", color: "#F4A460" }
	];
